...
PredType  BPNotTaken::predict(const Instruction * inst, InstID oracleID, bool doUpdate)
{
    ...
    if(inst->calcNextInstID() == oracleID)
    {
	updateBranchInstTracker(inst, true, false);
	return CorrectPrediction;
    }else{ 
	updateBranchInstTracker(inst, false, true);
	return MissPrediction;
    }
...
BPHybrid::~BPHybrid()
{
    // Finalize fn would use this above information. This will be called when say predictor is destroyed. 
    // Just before destrying we will dump the information from our new MAP. This code will parse through 
    // MAP and will track all the nTotals and create buckets - 20, 20-199, 200-1999, 2000+.
    // this new Map will be <1-19, SUM(nTotals), SUM(nMiss), SUM(nHit)> where nTotals between 1-19 
    // Report out for overall working.
}

// This is called by doPredict
PredType BPHybrid::predict(const Instruction *inst, InstID oracleID, bool doUpdate)
{
    bpredEnergy->inc();

    // btb prediction as an unconditional branch so always taken
    if( inst->isBranchTaken() )
    {	
	updateBranchInstTracker(inst,true, false);
        return btb.predict(inst, oracleID, doUpdate);
    ...
    // Now activate meta predictor to choose between 
    bool taken = (inst->calcNextInstID() != oracleID);
    HistoryType iID     = calcInstID(inst);
    ...
    // update historyTable statistics
    if (doUpdate) {
        ghr = ((ghr << 1) | ((iID>>2 & 1)^(taken?1:0))) & historyMask; // Since historyMask is 7, hence a 3 bit value.
    }
    ...
    /*if(ptaken)
    {
      PredType predType = btb.predict(inst, oracleID, doUpdate);
