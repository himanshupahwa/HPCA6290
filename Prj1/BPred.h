...
class BPred {
    std::map<uint32_t,BranchTypeStats*> BranchInstTracker;
public:
...
protected:
    ...
    bool activeBPred; // no of branch instructions
    ...
    /**
     * Initialize BranchTracker Map	
     * Update Branch Inst tracker with apt values.
    */
    BranchTypeStats* getBranchStatsForID(const Instruction* inst)
    {
	BranchTypeStats* brStats = BranchInstTracker.find(inst->addr) != BranchInstTracker.end() ? BranchInstTracker[inst->addr]:nullptr ;
        if(!brStats)
        {
            //std::cout<<"New Inst ID(PC)-->"<<inst->addr<<","<<inst->subCode<<std::endl;
            brStats = new BranchTypeStats();
            BranchInstTracker[inst->addr] = brStats;
            //brStats->total++;	
        } 
	return brStats;
    }

    void updateBranchInstTracker(const Instruction* inst, bool correctPredCond, bool incorrectPredCond)
    {
	BranchTypeStats* brStats = getBranchStatsForID(inst);
	brStats->correctPred+=correctPredCond;
	brStats->incorrectPred+=incorrectPredCond;
	brStats->total++;
    }	
protected:
public:
    ...
    void ReportOutAndCleanUp()
    {
	BranchTypeStats bucket[4];
	std::cout<<"Distinct Branch Instructions (static instruction count)---->"<<BranchInstTracker.size()<<std::endl;
	for(std::map<uint32_t,BranchTypeStats*>::iterator it=BranchInstTracker.begin(); it != BranchInstTracker.end();it++)
	{
	    BranchTypeStats* brStats = it->second;
	    //std::cout<<it->first<<","<<brStats->correctPred<<","<<brStats->incorrectPred<<","<<brStats->total<<std::endl;
	    if(brStats->total >= 1 && brStats->total <=19)
	    {
	    	bucket[0].total++;
	    	bucket[0].correctPred+=brStats->correctPred;
	    	bucket[0].incorrectPred+=brStats->incorrectPred;
	    }
	    else if(brStats->total >= 20 && brStats->total <=199)
	    {
	    	bucket[1].total++;
	    	bucket[1].correctPred+=brStats->correctPred;
	    	bucket[1].incorrectPred+=brStats->incorrectPred;
	    }
	    else if(brStats->total >= 200 && brStats->total <=1999)
	    {
	    	bucket[2].total++;
	    	bucket[2].correctPred+=brStats->correctPred;
	    	bucket[2].incorrectPred+=brStats->incorrectPred;
	    }
	    else if(brStats->total >= 2000)
	    {
	    	bucket[3].total++;
	    	bucket[3].correctPred+=brStats->correctPred;
	    	bucket[3].incorrectPred+=brStats->incorrectPred;
	    }

	
	    delete(it->second);
	}
	if(BranchInstTracker.size() > 0)
	{
	        // Report out consolidated  results
		int allBranchCounts = 0;
		std::string bucketStr[4] = {"[1-19]","[20-199]","[200-1999]","[2000+]"};
		for(int i = 0; i < 4; i++)
		{
			//if(!bucket[i])
			//{
				int total = bucket[i].total;
				int cPred = bucket[i].correctPred;
				int incPred = bucket[i].incorrectPred;
				double accuracy = (100.0*((double)bucket[i].correctPred/(double)(bucket[i].correctPred + bucket[i].incorrectPred)));
	        		std::cout<<bucketStr[i]<<total<<","<<cPred<<","<<incPred<<","<<accuracy<<std::endl;
				allBranchCounts+=bucket[i].total;
			//}
		} 
	        std::cout<<"TOTAL:"<<allBranchCounts<<std::endl;
	}
    }
