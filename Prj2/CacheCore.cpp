...
*CacheAssoc<State, Addr_t, Energy>::findLine2Replace(Addr_t addr, bool ignoreLocked)
{
    ...
    Line **nextLineFree=0; // Order of preference, invalid, locked
    Line **setEnd = theSet + assoc;
    ...
    // and the oldest isLocked possible (lineFree)
    {
            ...
            if(lineFree != 0)
	    {
            	if (!(*l)->isValid())
                	nextLineFree = l;
            	else if (nextLineFree == 0 && !(*lineFree)->isValid())
                	nextLineFree = lineFree;
            	else if (nextLineFree == 0 && !(*l)->isLocked())
			nextLineFree = l;
	    }
    ...
    {
    	if(nextLineFree == 0 && !ignoreLocked)
	{
    		if(lineFree == 0 && !ignoreLocked)
        		return 0;
		else
