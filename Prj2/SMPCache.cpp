...
bool SMPCache::pushToFACache(PAddr addr,bool& isNewElement,bool& isElementInFACache)
{
	...
	ReplacementPolicy policy = LRU;
        ...
        // Always push in Infinte Cache 
        isNewElement = InfiniteCache.find(taddr) == InfiniteCache.end();
        InfiniteCache[taddr]=isNewElement ? 0 : InfiniteCache[taddr]+1;
    
        // Always look up in FACache.
        std::list<PAddr>::iterator it = std::find(FACacheQ.begin(), FACacheQ.end(), taddr);
        isElementInFACache = FACacheQ.end() != it;
        if(!isElementInFACache)
        {
                if(FACacheQ.size() == cache->getNumLines())
                {
			//Evict
			if(policy == LRU)
			{
			...
			{
				PAddr copyLRUElement = FACacheQ.back();
				FACacheQ.pop_back();	
				PAddr taddrToDelete = FACacheQ.back();
				FACacheQ.pop_back();	
				// Get the LRU element back at the position.
				FACacheQ.push_back(copyLRUElement);
			}
                }
        }
        else
...
bool SMPCache::pushToFACacheFASTER(PAddr addr, bool& isNewElement, bool& elementInFACache)
{
	// Extracting policy to make our FACache follow LRU or NXLRU if needed
	//ReplacementPolicy policy = cache->getReplacementPolicy();
	ReplacementPolicy policy = LRU;
	//std::cout<<"POLICY LOCAL:"<<policy<<"Cache Policy:"<<cache->getReplacementPolicy()<<std::endl;
	...
	// Always look up in FACache.
	elementInFACache = FACache.find(taddr) != FACache.end();
	if(!elementInFACache)
	{
		//std::cout<<"Cache Size:"<<FACache.size()<<" QueueSize:"<<FACacheQ.size()<<" Cache Numlines:"<<cache->getNumLines()<<std::endl;

		// based on size of cache and policy pop the element
		if(FACacheQ.size() == cache->getNumLines())
		{
			//Evict
			if(policy == LRU)
			{
				...
				PAddr taddrToDelete = FACacheQ.back();
				FACacheQ.pop_back();	
				FACache.erase(taddrToDelete);
			}
			else if(policy == NXLRU)
			{
				//std::cout<<"NXLRU POLICY :"<<policy<<std::endl;
				PAddr copyLRUElement = FACacheQ.back();
				FACacheQ.pop_back();	
				PAddr taddrToDelete = FACacheQ.back();
				FACacheQ.pop_back();	
				...
				// Get the LRU element back at the position.
				FACacheQ.push_back(copyLRUElement);
				list<PAddr>::iterator iter = FACacheQ.begin();
			        // The very reason we created map is now a bottleneck in NXLRU as we advance iterator to end +O(n) time
				// possibly a different data structure othr than queue would have worked better. Basicaaly STL DS 
 				// with iterator to tail element.
				std::advance(iter, FACacheQ.size() - 1); //iter is set to last element
				FACache[copyLRUElement] = iter;
			}
		}
	}
	else
	{
		FACacheQ.erase(FACache[taddr]);	
	}
	FACacheQ.push_front(taddr);
	FACache[taddr] = FACacheQ.begin();
...
void SMPCache::populateMissTypeCount(bool isNewElement,bool isElementInFACache)
{
    if(isNewElement)
    {
    	compMiss.inc();
    }
    else if(isElementInFACache)
    {
    	confMiss.inc();
    }
    else
...
void SMPCache::doRead(MemRequest *mreq)
{
    ...
    if (l && l->canBeRead()) {
        ...
        readHit.inc();
    	// Intercept FA Cache code here
    	//bool success = pushToFACache(addr, isNewElement, isElementInFACache);
    	bool success = pushToFACacheFASTER(addr, isNewElement, isElementInFACache);
	I(success);
#ifdef SESC_ENERGY
        rdEnergy[0]->inc();
#endif
    ...
    I( success );
    readMiss.inc();
    populateMissTypeCount(isNewElement,isElementInFACache);

     // Get the TAG associated with the address we trying to access and it got to miss
     // Check if TAG is unique and has not been see before. If not then  increment compMiss
