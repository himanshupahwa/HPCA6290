...
public:
    ...
    CacheLine *findPrevLine(Addr_t addr) {
        return findPrevLinePrivate(addr);
    }
    ...
    CacheLine *readPrevLine(Addr_t addr) {
        IS(goodInterface=true);
        CacheLine *line = findPrevLine(addr);
        IS(goodInterface=false);
        return line;
    }
    ...
    CacheLine *writePrevLine(Addr_t addr) {
        IS(goodInterface=true);
        CacheLine *line = findPrevLine(addr);
        IS(goodInterface=false);
        return line;
...
protected:
    ...
    Line *findPrevLinePrivate(Addr_t addr);
public:
    virtual ~CacheAssoc() {
        delete [] content;
        delete [] mem;
...
public:
    ...
    /**
	Previous tag tracking functions.
    */
    Addr_t getPrevTag() const {
        return prevTag;
    }
    bool getPrevTagValid()
    {
	return wasValid;
    }	
    ...
    void setTag(Addr_t a) {
        I(a);
	if(!tag) // Is the tag already invalidated. Dont let prevTag have the after effect.
	{
		prevTag = tag;
		wasValid = (tag != 0);
	}
