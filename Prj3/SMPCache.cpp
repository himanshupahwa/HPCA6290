...
void SMPCache::populateReadCohMissTypeCount(bool canNotBeRead,bool isNewElement, PAddr addr)
{
    if(isNewElement)
    {
	readCompMiss.inc();
	...
	{
		Line *lf = cache->findPrevLine(addr);
		if(lf)
		{
			if(!lf->isValid())
			{
				if(lf->getPrevTagValid())
				{
					readCoheMiss.inc();
				}
				else
...
void SMPCache::populateWriteCohMissTypeCount(bool canNotBeWritten,bool isNewElement, PAddr addr)
{
    if(isNewElement)
    {
	writeCompMiss.inc();
...
void SMPCache::doRead(MemRequest *mreq)
{
    ...
    populateReadCohMissTypeCount(l && !(l->canBeRead()),isNewElement,addr);	
...
void SMPCache::doWrite(MemRequest *mreq)
{
    ...
    bool success = pushToFACacheFASTER(addr, isNewElement, isElementInFACache);
    I( success );
    populateWriteCohMissTypeCount(l && !(l->canBeWritten()),isNewElement,addr);	
    populateMissTypeCount(isNewElement,isElementInFACache);	
