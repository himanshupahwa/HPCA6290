...
public:
    ...
    // Global Address Cache for compulsary misses
    // For debug purposes we keep track of count of all the addresses that
    // are ever read or written to. Hence the value of type is integer(count)
    std::map<PAddr,int> InfiniteCache;

    // FA Cache for conflict and capacity count.
    // Keeps the tag mapping to count, as to how many
    // times an element was observed in the cache.

    // This is TAG locator via map. This is definately quicker as maps operate on hashing.
    // We store the list pointer where it is located in queue(FIFO). The queue is used for
    // eviction approach. Using this faster however more mem used as N elements in map
    // and N elements in FIFO
    std::map<PAddr,std::list<PAddr>::iterator> FACache;
    // This is the FIFO used for our operations. We can merely use FIFO or use it in conjunction with
    // map FACache.
    std::list<PAddr> FACacheQ;
    // Uses only FIFO as priority queue approach. Least recently used element only available at back and
    // Most recently used element at front
    // This function uses both FIFO and Map for faster execution but 2N space in camparison to above implementation.
    bool pushToFACacheFASTER(PAddr addr, bool& isNewElement, bool& elementInFACache);
    void populateMissTypeCount(bool isNewElement,bool isElementInFACache);
    void populateReadCohMissTypeCount(bool cannotBeRead, bool isNewElement, PAddr addr);
    void populateWriteCohMissTypeCount(bool cannotBeWritten, bool isNewElement, PAddr addr);
